...
public class LLIndexList1<E> implements IndexList<E> {
	private LinkedList<E> internalLL;  

	/**
		Creates an empty instance of a list. 
	 **/ 
	public LLIndexList1(LinkedList<E> theList) 
	{ 
		internalLL = theList;
	}
	 ...
	 **/
	private Node<E> getDataINodeAtPosition(int posIndex)
	{ 
	...
	throws IndexOutOfBoundsException 
	{
		...
		if (index==0) 
			...
			Node<E> nodoPrevio = 
				getDataINodeAtPosition(index-1); 
	...
	public E get(int index) throws IndexOutOfBoundsException {
		...
		Node<E> targetINode = this.getDataINodeAtPosition(index);
	...
	public E remove(int index) throws IndexOutOfBoundsException {
		...
		Node<E> ntr = this.getDataINodeAtPosition(index); 
	...
	public E set(int index, E e) throws IndexOutOfBoundsException {
		...
		Node<E> ntc = this.getDataINodeAtPosition(index); 
	...
	public void add(E value) {
		Node<E> nn = internalLL.createNewNode(); 
		nn.setElement(value); 
		
		if (internalLL.length() == 0)
			internalLL.addFirstNode(nn); 
		else { 
			Node<E> last = internalLL.getLastNode(); 
			internalLL.addNodeAfter(last, nn); 
		}
